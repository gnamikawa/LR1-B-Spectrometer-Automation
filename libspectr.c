...
{   
    ...
    if (g_Device) {
        printf("HID Device already connected. Disconnecting.\n");
        disconnectDevice();
    }
    ...
    if (!g_Device) {
        ...
        printf("Failed to connect to the HID.\n");
...
int setAllParameters(u_int16_t numOfScans, u_int16_t numOfBlankScans, u_int8_t scanMode, u_int32_t timeOfExposure, u_int8_t enableMode, u_int8_t signalFrontMode)
{
    ...
    result = _writeReadFunction(report, CORRECT_SET_ALL_PARAMETERS_REPLY, STANDARD_TIMEOUT_MILLISECONDS);
